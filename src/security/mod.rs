...
impl SecurityGuard {
    ...
    fn load_default_rules(&mut self) {
        let rules = vec![
            SecurityRule {
                ...
                pattern: r"(?i)(aws_access_key|aws_secret_key|AMAZON|S3_BUCKET)".to_string(),
            ...
            SecurityRule {
                ...
                pattern: r#"(?i)(api[_-]?key|apikey|API_KEY)[=\s]*['"]?[a-zA-Z0-9]{20,}['"]?"#.to_string(),
            ...
            SecurityRule {
                ...
                pattern: r"eyJ[a-zA-Z0-9]{10,}\.eyJ[a-zA-Z0-9_-]{10,}\.[a-zA-Z0-9_-]{10,}".to_string(),
            ...
            SecurityRule {
                ...
                pattern: r"(rm\s+-rf\s+/|mkfs|dd\s+if=)".to_string(),
                severity: Severity::Critical,
    ...
    pub async fn audit_directory(&self, path: &Path) -> Result<AuditReport> {
        ...
        let critical_count = findings.iter().filter(|f| f.rule.severity == Severity::Critical).count();
        let high_count = findings.iter().filter(|f| f.rule.severity == Severity::High).count();
    ...
    pub async fn validate_command(&self, command: &str) -> CommandValidation {
        let dangerous_patterns = [
            (r"rm\s+-rf\s+/(?:\*|var|etc|usr|home)", "Deleting system directories"),
    ...
    async fn log_event(&self, event: SecurityEvent, file: Option<String>, severity: Severity, details: &str) {
...
impl TddEnforcer {
    pub fn new(enabled: bool) -> Self {
        ...
        frameworks.insert("typescript".to_string(), TestFramework {
    ...
    pub async fn enforce(&self, project_path: &Path, source_files: Vec<PathBuf>) -> Result<TddReport> {
        if !self.enabled {
            return Ok(TddReport {
                tests_generated: 0,
                tests_passed: 0,
        ...
        let report = TddReport {
            ...
            details: format!("Detected {} project with {} framework", lang, framework.map(|f| &f.name[..]).unwrap_or("unknown")),
        };
